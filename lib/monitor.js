...
class Monitor {
    constructor(server, options) {
        ...
        if (this.settings.wreck) {
            this._attachWreckPayload = (uri = {}, options = {}) => {

                try {
                    uri.payload = JSONBigInt.parse(options.payload); // eslint-disable-line
                } catch (err) {
                    uri.payload = options.payload;
                }
            };

            this._wreckHandler = (error, { req, res, start, uri }) => {

                if (!error && res) {
                      Wreck.read(res, null, (readError, payloadBuffer) => {
                            let payload = undefined;
                            if (!readError) {
                                  try {
                                        payload = JSONBigInt.parse(payloadBuffer.toString());
                                  } catch (err) {
                                        server.log('warn', `Failed to parse wreck payload from ${uri.path}: ${err}`);
                                  }
                            }
                            this.push(() => new Utils.WreckResponse(error, req, res, start, uri, payload));
                      });
                } else {
                    this.push(() => new Utils.WreckResponse(error, req, res, start, uri ));
                }
    ...
    start(callback) {
        ...
        if (this.settings.wreck) {
            ...
            process[wreck].on('request', this._attachWreckPayload);
